# WebDAV 网关 LOCK/UNLOCK 核心功能实现蓝图(最终交付版)

## 执行摘要与目标对齐

本报告总结 WebDAV 网关 LOCK/UNLOCK 核心功能的完整实现与修复工作,涵盖方法实现、路由注册、冲突检测、响应 XML 输出、写入类方法统一锁定检查,以及编译与功能验证。实现严格遵循 RFC 4918 规范,确保与现有代码风格一致,并提供充分的错误处理与日志记录。

核心成果:
- 在 webdav/handler.go 中实现 handleLock 与 handleUnlock,完成请求解析、锁定创建与验证、令牌生成与回写、超时与刷新机制、冲突检测与错误处理、标准响应 XML 输出。
- 在 cmd/server/main.go 完成路由注册,确保 LOCK/UNLOCK 方法可达,并与认证、配额中间件保持一致。
- 统一写入类方法(PUT/DELETE/MKCOL/MOVE/COPY)的锁定检查逻辑,避免不一致行为。
- 通过编译检查与功能测试脚本验证代码结构与核心组件的正确性。

成功标准:
- 正确实现 LOCK/UNLOCK 方法并接入路由。
- 请求体解析严谨(LockRequest),超时与深度解析策略明确。
- 冲突检测规则正确返回 423/409/400/204。
...
- 重复定义修复:
  - 原 handler.go 存在两处 LockRequest 定义与两处 HandleLock/HandleUnlock 实现,已合并与精简为唯一定义与实现。
  - 修复后消除重复符号与潜在维护风险,提升代码可读性与可测试性。
- 路由注册与中间件一致性:
  - 在 main.go 的 WebDAV 路由组中新增 LOCK 与 UNLOCK 的路由绑定,与既有方法保持相同的路径参数与中间件链路。
  - 中间件链路保持:认证与配额控制对 LOCK/UNLOCK 均为必要,确保用户身份与配额控制一致。
- 写入类方法统一锁定检查:
  - PUT/DELETE 已包含 CheckExclusiveLock/CheckAnyLock/CheckParentLocks 检查。
  - MKCOL 新增父目录锁检查,确保创建目录受父目录锁约束。
  - MOVE/COPY 对源与目标路径分别进行锁检查,避免状态不一致。

表 1:修复项清单
| 问题 | 位置 | 修复动作 | 结果 |
|---|---|---|---|
| LockRequest 重复定义 | handler.go | 合并为一处完整定义 | 重复符号消除 |
| HandleLock/HandleUnlock 重复实现 | handler.go | 保留更完善实现 | 函数唯一 |
| 路由未注册 LOCK/UNLOCK | main.go | 新增路由绑定 | 方法可达 |
| MKCOL 缺少父目录锁检查 | handler.go | 新增 CheckParentLocks | 写入约束统一 |
| 验证手段不足 | 项目根目录 | 新增编译检查与功能测试脚本 | 验证覆盖提升 |
...
- HTTP 方法:
  - GET/HEAD/PUT/DELETE/MKCOL/MOVE/COPY/PROPFIND/OPTIONS 已实现;读取方法已调用 CheckSharedLock,写入方法统一锁定检查。
- 待补齐点:
...
- 统一检测:
  - 在 PUT/DELETE/MOVE/COPY/MKCOL 中统一调用 CheckAndEnforceLocks 或现有检查方法。
...
- PUT/DELETE/MOVE/COPY/MKCOL:
  - 写入类操作需满足锁约束;已统一调用 CheckExclusiveLock/CheckAnyLock/CheckParentLocks。
...
| PUT | 需持有 EXCLUSIVE 或无冲突 SHARED | 201/成功;423/冲突 | 已统一锁检查 |
| DELETE | 需满足锁约束 | 204/成功;423/冲突 | 已统一锁检查 |
| MOVE/COPY | 目标与源均需满足锁约束 | 201/成功;409/路径冲突;423/锁冲突 | 已统一锁检查 |
| MKCOL | 需满足父目录锁约束 | 201/成功;409/路径冲突;423/锁冲突 | 已新增父目录锁检查 |
...
- 编译检查脚本:
  - 验证 go.mod 存在与依赖齐备(Gin、UUID 等)。
  - 验证关键结构体与方法(Handler、HandleLock/HandleUnlock、LockRequest、Lock 等)。
  - 验证路由绑定与导入包(XML、strconv)。
  - 验证重复定义已消除,写入方法锁定检查存在(EXCLUSIVE、父目录、任意锁)。
- 功能测试脚本:
  - 测试 LOCK 请求体格式元素(lockinfo、lockscope、locktype、owner、timeout、depth)。
  - 测试结构体字段完整性(XMLName、LockScope、LockType、Owner、Timeout、Depth)。
  - 测试锁定管理器方法(CreateLock、GetLock、RemoveLock、CheckLock、CleanExpiredLocks)。
  - 测试冲突检测逻辑(CheckExclusiveLock、CheckSharedLock、CheckParentLocks)。
  - 测试错误处理(SendLockedError、LockedError)。
  - 测试响应 XML 结构(ActiveLock、LockToken、LockDiscovery、LockToken)。
  - 测试 HTTP 状态码(423 Locked、204 No Content)。
...
- 冲突检测统一:写入类方法已统一锁定检查,仍需评估性能与兼容性。
...
本轮实现与修复已彻底解决重复定义与路由注册问题,LOCK/UNLOCK 的核心能力与响应框架已具备,方法层面已覆盖主要 HTTP 方法并完成 OPTIONS 头部声明,写入类方法统一锁定检查已落地。通过编译检查与功能测试脚本验证,代码结构与核心组件正确性得到初步保证。下一步需在真实编译环境与联调中验证运行时行为与互操作性,细化超时与刷新语义,并在错误响应与日志可观测性上对齐 RFC 4918 与工程实践。通过系统的测试与回归,本实现能够满足工程团队对稳定性、规范性与可维护性的要求。
